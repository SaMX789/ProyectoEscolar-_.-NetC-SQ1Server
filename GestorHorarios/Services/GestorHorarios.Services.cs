...
namespace GestorHorarios.Services
{
    public class GeneradorHorariosService
    {
        public async Task<string> EjecutarDiagnosticoAsync(int idProyecto, int idCarrera)
        {
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    // Maestro virtual (ID -1) para absorber materias sin profesor real
                    ...
                    // Diccionario para BALANCEO DE CARGA
                    Dictionary<int, int> horasAsignadasPorMaestro = new Dictionary<int, int>();
                    foreach (var doc in docentes) horasAsignadasPorMaestro[doc.IdDocente] = 0;

                    // 1. ASIGNACIÓN INTELIGENTE (Load Balancing y Turnos)
                    foreach (var g in grupos)
                    {
                        ...
                        foreach (var m in materiasDelGrupo)
                        {
                            ...
                            // Filtramos maestros que tengan al menos 1 bloque libre que coincida con el turno del grupo
                            var docentesDisponibles = docentesCapaces.Where(d =>
                                d.DisponibilidadBloques.Count == 0 || // Si no tiene configuración, asume que puede
                                d.DisponibilidadBloques.Any(disp => bloquesDelGrupo.Contains(int.Parse(disp.Split('_')[1])))
                            ).ToList();

                            var pool = docentesDisponibles.Count > 0 ? docentesDisponibles : docentesCapaces;

                            Docente docenteElegido = null;
                            if (pool.Count > 0)
                            {
                                // BALANCEO: Elegimos al maestro capaz que tenga MENOS horas asignadas
                        ...
                        foreach (var m in materiasDelGrupo)
                        {
                            ...
                            foreach (var d in dias)
                            {
                                for (int i = 0; i < bloquesPermitidos.Count; i++)
                                {
                                    ...
                                    if (idMaestro != -1 && tieneDisponibilidadConfigurada)
                                    {
                                        if (!docente.DisponibilidadBloques.Contains($"{d}_{b}"))
                                        {
                    ...
                    // 3. RESTRICCIONES DE LOS ALUMNOS
                    foreach (var g in grupos)
                    {
                        var materiasDelGrupo = materias.Where(m => m.Semestre == g.Semestre).ToList();
                        var bloquesPermitidos = bloquesPorGrupo[g.IdGrupo];
                        ...
                        foreach (var m in materiasDelGrupo)
                        {
                            ...
                            // MEJOR ESFUERZO: Permite dejar huecos, pero lo penaliza fuertemente
                    ...
                    // 4. RESTRICCIONES ESTRICTAS DE PROFESORES
                    foreach (var doc in docentes)
                    {
                        ...
                        if (clasesSemanalesDocente.Count > 0)
                            model.Add(LinearExpr.Sum(clasesSemanalesDocente) <= doc.HorasMaximas);
                    }
                    ...
                    if (status == CpSolverStatus.Optimal || status == CpSolverStatus.Feasible)
                    {
                        ...
                        return $"EXITO: Horario generado (Mejor esfuerzo). Se insertaron {insertados} clases. Las materias que rebasan los límites de tiempo o personal se dejaron como espacios libres.";
                    }
                    else if (status == CpSolverStatus.Infeasible)
                    {
                        return "ERROR: Las reglas están tan ajustadas que ni siquiera haciendo el mejor esfuerzo se pudo calcular. Revisa los datos base.";
                    }

                    return $"ERROR: {status} - El tiempo de cálculo se agotó.";
                }
                catch (Exception ex)
                {
        ...
        private int GuardarResultadosEnDB(int idProyecto, int idCarrera, List<Grupo> grupos, List<Materia> materias, int[] dias, Dictionary<int, List<int>> bloquesPorGrupo, Dictionary<string, int> materiaDocenteAsignado, Dictionary<string, BoolVar> asignaciones, CpSolver solver)
        {
            ...
            // SQL SEGURO: Borra usando una subconsulta para jamás afectar a otras carreras
            using (var cmdDelete = new SqlCommand(@"
                DELETE FROM HorarioDetalle 
                WHERE id_proyecto = @idProj 
                AND id_grupo IN (SELECT id_grupo FROM Grupos WHERE id_carrera = @idCarrera)", conn))
            {
